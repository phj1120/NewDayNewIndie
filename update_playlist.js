  ...
  async getLatestVideos(channelId) {
    try {
      ...
      if (error.code === 403 && error.message.includes("quotaExceeded")) {
        Logger.error(
          "YouTube API 할당량이 초과되었습니다. 24시간 후에 다시 시도해주세요."
        );
      } else {
        Logger.error(
          `채널 ${channelId}의 동영상을 가져오는데 실패했습니다: ${error.message}`
        );
      }
      return [];
    }
...
async function updatePlaylist() {
  ...
  try {
    ...
    await playlistManager.updatePlaylistItems(
